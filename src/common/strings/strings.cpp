...
const char *AddStaticString(const char *String){
	...
	if((StringLen + 1) > StaticBlockSize){
		error("AddStaticString: String too long (%d).\n", StringLen);
		return NULL;
	}
...
uint32 AddDynamicString(const char *String){
	...
	if((StringLen + 1) > DynamicBlockSize){
		error("AddDynamicString: String too long (%d).\n", StringLen);
		return 0;
	}
	...
	if(EntryIndex == -1){
		error("AddDynamicString: No free slot found.\n");
		return 0;
	}
...
const char *GetDynamicString(uint32 Number){
	...
	if(Node == NULL){
		error("GetDynamicString: Block for string %u does not exist.\n", Number);
		return NULL;
	}

	TDynamicStringTableBlock *Block = &Node->data;
	if(Block->EntryType[EntryIndex] != DYNAMIC_STRING_ALLOCATED){
		error("GetDynamicString: Entry for string %u does not exist.\n", Number);
		return NULL;
	}
...
void DeleteDynamicString(uint32 Number){
	...
	if(Node == NULL){
		error("DeleteDynamicString: Block for string %u does not exist.\n", Number);
		return;
	}

	TDynamicStringTableBlock *Block = &Node->data;
	if(Block->EntryType[EntryIndex] != DYNAMIC_STRING_ALLOCATED){
		error("DeleteDynamicString: Entry for string %u does not exist.\n", Number);
		return;
	}
		...
		for(int i = 0; i < DynamicBlockEntries; i += 1){
			if(Block->EntryType[i] == DYNAMIC_STRING_DELETED){
				...
				if(StringEnd > DynamicBlockSize){
					error("CleanupDynamicStrings: String end is missing.\n");
					continue;
				}
...
const char *Plural(const char *s, int Count){
	...
	if(s == NULL){
		error("Plural: Provided string does not exist.\n");
...
const char *SearchForWord(const char *Pattern, const char *Text){
	if(Pattern == NULL || Pattern[0] == 0){
		error("SearchForWord: Provided search word does not exist.\n");
		return NULL;
	}

	if(Text == NULL){
		error("SearchForWord: Provided text does not exist.\n");
		return NULL;
	}
...
const char *SearchForNumber(int Count, const char *Text){
	if(Count < 1){
		error("SearchForNumber: Illegal search number %d.\n", Count);
		return NULL;
	}

	if(Text == NULL){
		error("SearchForNumber: Provided text does not exist.\n");
		return NULL;
	}
...
bool MatchString(const char *Pattern, const char *String){
	if(Pattern == NULL){
		error("MatchString: Pattern is NULL.\n");
		return false;
	}

	if(String == NULL){
		error("MatchString: String is NULL.\n");
		return false;
	}
...
void AddSlashes(char *Destination, const char *Source){
	if(Source == NULL){
		error("AddSlashes: Source is NULL.\n");
		return;
	}

	if(Destination == NULL){
		error("AddSlashes: Destination is NULL.\n");
		return;
	}
...
void Trim(char *Destination, const char *Source){
	if(Source == NULL){
		error("Trim: Source is NULL.\n");
		return;
	}

	if(Destination == NULL){
		error("Trim: Destination is NULL.\n");
		return;
	}
...
void ChangeWildcards(char *String){
	if(String == NULL){
		error("ChangeWildcards: String is NULL.\n");
		return;
	}
