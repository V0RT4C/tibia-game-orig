...
TPlayer::TPlayer(TConnection *Connection, uint32 CharacterID)
	: TCreature(), AttackedPlayers(0, 10, 10), FormerAttackedPlayers(0, 10, 10) {
	this->Type = PLAYER;
	...
	for (int SpellNr = 0; SpellNr < NARRAY(this->SpellList); SpellNr += 1) {
		this->SpellList[SpellNr] = 0;
	}

	for (int QuestNr = 0; QuestNr < NARRAY(this->QuestValues); QuestNr += 1) {
		this->QuestValues[QuestNr] = 0;
	}

	for (int ContainerNr = 0; ContainerNr < NARRAY(this->OpenContainer); ContainerNr += 1) {
		this->OpenContainer[ContainerNr] = NONE;
	}

	static_assert(NARRAY(this->Addressees) == NARRAY(this->AddresseesTimes));
	for (int AddresseeNr = 0; AddresseeNr < NARRAY(this->Addressees); AddresseeNr += 1) {
	...
	if (HouseID != 0 && !is_invited(HouseID, this, PlayerData->LastLogoutTime) &&
...
TPlayer::~TPlayer(void) {
	...
	if (!this->IsDead) {
		...
		if (ResetCharacter) {
			...
			for (int SpellNr = 0; SpellNr < NARRAY(PlayerData->SpellList); SpellNr += 1) {
				PlayerData->SpellList[SpellNr] = 0;
			}

			for (int QuestNr = 0; QuestNr < NARRAY(PlayerData->QuestValues); QuestNr += 1) {
				PlayerData->QuestValues[QuestNr] = 0;
			}

			// NOTE(fusion): This is used to reset skills back to default. See
			// `TPlayer::LoadData`.
			for (int SkillNr = 0; SkillNr < NARRAY(PlayerData->Minimum); SkillNr += 1) {
...
bool TPlayer::MovePossible(int x, int y, int z, bool Execute, bool Jump) {
	...
	if (Result && Execute) {
		if (this->EarliestProtectionZoneRound > RoundNr && is_protection_zone(x, y, z) &&
			!is_protection_zone(this->posx, this->posy, this->posz)) {
			throw ENTERPROTECTIONZONE;
		}

		uint16 HouseID = get_house_id(x, y, z);
		if (HouseID != 0 && !is_invited(HouseID, this, INT_MAX) && !check_right(this->ID, ENTER_HOUSES)) {
			throw NOTINVITED;
		}
	}
	return Result;
...
void TPlayer::LoadData(void) {
	...
	for (int SkillNr = 0; SkillNr < NARRAY(this->Skills); SkillNr += 1) {
		...
		this->Skills[SkillNr]->Load(
			PlayerData->Actual[SkillNr], PlayerData->Maximum[SkillNr], PlayerData->Minimum[SkillNr],
			PlayerData->DeltaAct[SkillNr], PlayerData->MagicDeltaAct[SkillNr], PlayerData->Cycle[SkillNr],
			PlayerData->MaxCycle[SkillNr], PlayerData->Count[SkillNr], PlayerData->MaxCount[SkillNr],
			PlayerData->AddLevel[SkillNr], PlayerData->Experience[SkillNr], PlayerData->FactorPercent[SkillNr],
			PlayerData->NextLevel[SkillNr], PlayerData->Delta[SkillNr]);
	}
}
...
void TPlayer::LoadInventory(bool SetStandardInventory) {
	...
	} else if (SetStandardInventory && this->Profession == PROFESSION_NONE && !check_right(this->ID, ZERO_CAPACITY)) {
		try {
			create(get_body_container(this->ID, INVENTORY_RIGHTHAND), get_special_object(DEFAULT_RIGHTHAND), 0);
			create(get_body_container(this->ID, INVENTORY_LEFTHAND), get_special_object(DEFAULT_LEFTHAND), 0);
			if (this->Sex == 1) {
				create(get_body_container(this->ID, INVENTORY_TORSO), get_special_object(DEFAULT_BODY_MALE), 0);
			} else {
				create(get_body_container(this->ID, INVENTORY_TORSO), get_special_object(DEFAULT_BODY_FEMALE), 0);
			}

			Object Bag = create(get_body_container(this->ID, INVENTORY_BAG), get_special_object(DEFAULT_CONTAINER), 0);
	...
	for (int i = 0; i < 2; i += 1) {
		...
		try {
			...
			if (!check_right(Player[Other]->ID, UNLIMITED_CAPACITY)) {
				...
				int FinalWeight = get_inventory_weight(Player[Other]->ID) + get_complete_weight(Obj[Cur]);
...
bool TPlayer::GetActivePromotion(void) {
	return check_right(this->ID, PREMIUM_ACCOUNT) && this->Profession >= PROFESSION_PROMOTION;
}
...
void TPlayer::CheckState(void) {
	if (this->Connection != NULL) {
		...
		if (this->Skills[SKILL_DRUNKEN]->TimerValue() > 0 && this->Skills[SKILL_DRUNKEN]->Get() == 0) {
			State |= 0x08;
		}
...
bool TPlayer::IsAggressor(bool CheckFormer) {
	return this->Aggressor || (CheckFormer && this->FormerAggressor && (this->FormerLogoutRound + 5) >= RoundNr);
}
...
bool TPlayer::IsAttackJustified(uint32 VictimID) {
	...
	if (WorldType != PVP_ENFORCED // TODO(fusion): Probably `WorldType == NORMAL` ?
		&& Victim->PlayerData != NULL && Victim->PlayerData->PlayerkillerEnd == 0) {
...
void TPlayer::RecordAttack(uint32 VictimID) {
	...
	if (!Victim->InPartyWith(this, true) && !Victim->IsAttacker(this->ID, true) && !this->IsAttacker(VictimID, false)) {
...
void TPlayer::RecordMurder(uint32 VictimID) {
	if (WorldType != NORMAL || VictimID == this->ID || this->IsAttackJustified(VictimID)) {
		return;
	}
	...
	if (Playerkilling != 0) {
		...
		if (Playerkilling == 2) {
			...
			punishment_order(NULL, this->Name, this->IPAddress, 28, 2,
							 "Exceeding the limit of unjustified kills by 100%.", 0, NULL, 0, false);
		}
	}
}
...
int TPlayer::GetPartyMark(TPlayer *Observer) {
	...
	if (Observer->GetPartyLeader(false) == Observer->ID && is_invited_to_party(this->ID, Observer->ID)) {
		return PARTY_SHIELD_GUEST;
	}

	if (this->GetPartyLeader(false) == this->ID && is_invited_to_party(Observer->ID, this->ID)) {
...
void print_player_positions(void) {
	for (int Index = 0; Index < FirstFreePlayer; Index += 1) {
		...
		log_message("players", "[%d,%d,%d] %d %d\n", Player->posx, Player->posy, Player->posz,
					Player->Skills[SKILL_LEVEL]->Get(), Player->GetActiveProfession());
	}
}
...
void load_depot(TPlayerData *PlayerData, int DepotNr, Object Con) {
	...
	if (PlayerData->Depot[DepotNr] == NULL) {
		...
		try {
			TReadBuffer ReadBuffer(PlayerData->Depot[DepotNr], PlayerData->DepotSize[DepotNr]);
...
void get_profession_name(char *Buffer, int Profession, bool Article, bool Capitals) {
	...
	if (Article) {
		if (Profession == PROFESSION_ELITE_KNIGHT || Profession == PROFESSION_ELDER_DRUID) {
			strcat(Buffer, "an ");
		} else {
			strcat(Buffer, "a ");
		}
...
void PlayerDataPath(char *Buffer, int BufferSize, uint32 CharacterID) {
	snprintf(Buffer, BufferSize, "%s/%02u/%u.usr", USERPATH, (CharacterID % 100), CharacterID);
}
...
bool LoadPlayerData(TPlayerData *Slot) {
	...
	for (int SkillNr = 0; SkillNr < NARRAY(Slot->Minimum); SkillNr += 1) {
...
void SavePlayerData(TPlayerData *Slot) {
	...
	try {
		...
		for (int SpellNr = 0; SpellNr < NARRAY(Slot->SpellList); SpellNr += 1) {
		...
		for (int QuestNr = 0; QuestNr < NARRAY(Slot->QuestValues); QuestNr += 1) {
...
TPlayerData *assign_player_pool_slot(uint32 CharacterID, bool DontWait) {
	...
	if (Slot == NULL) {
		...
		for (int i = 0; i < NARRAY(PlayerDataPool); i += 1) {
			if (PlayerDataPool[i].Locked == 0 && PlayerDataPool[i].Sticky == 0 && !PlayerDataPool[i].Dirty) {
				Slot = &PlayerDataPool[i];
...
void save_player_pool_slots(void) {
	...
	for (int i = 0; i < NARRAY(PlayerDataPool); i += 1) {
		...
		if (Slot->CharacterID == 0 || Slot->Locked != 0 || Slot->Sticky > 0 || !Slot->Dirty) {
			continue;
		}
...
void insert_player_index(PlayerIndexInternalNode *Node, int Position, const char *Name, uint32 CharacterID) {
	while (true) {
		...
		for (int i = 0; i < ChildLeaf->Count; i += 1) {
			insert_player_index(ChildInternal, Position + 1, ChildLeaf->Entry[i].Name, ChildLeaf->Entry[i].CharacterID);
		}
...
void init_player_index(void) {
	...
	while (true) {
		int NumberOfPlayers;
		int Ret = QueryManager.loadPlayers(MinimumCharacterID, &NumberOfPlayers, Names, CharacterIDs);
		if (Ret != 0) {
