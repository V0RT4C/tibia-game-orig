...
void notify_monsterhome_of_death(int Nr) {
	...
	if (MH->ActMonsters >= MH->MaxMonsters) {
		error("notify_monsterhome_of_death: Monsterhome %d had too many monsters (%d instead of %d).\n", Nr,
			  (MH->ActMonsters + 1), MH->MaxMonsters);
		return;
	}
...
bool monsterhome_in_range(int Nr, int x, int y, int z) {
	...
	Monsterhome *MH = MonsterhomeList.at(Nr);
	return std::abs(z - MH->z) <= 2 && std::abs(x - MH->x) <= MH->Radius && std::abs(y - MH->y) <= MH->Radius;
}

// TMonster
// =============================================================================
TMonster::TMonster(int Race, int x, int y, int z, int Home, uint32 MasterID) : Nonplayer() {
	this->Type = MONSTER;
	this->Race = Race;
	this->startx = x;
	this->starty = y;
	...
	if (race_data[Race].Article[0] == 0) {
		...
		snprintf(this->Name, sizeof(this->Name), "%s %s", race_data[Race].Article, race_data[Race].Name);
	}
...
bool TMonster::MovePossible(int x, int y, int z, bool Execute, bool Jump) {
	...
	if (this->State != ATTACKING && this->State != PANIC) {
		...
		int Distance = std::max<int>(std::abs(x - this->posx), std::abs(y - this->posy));
...
bool TMonster::IsPeaceful(void) {
	return this->Master != 0 && is_creature_player(this->Master);
}
...
void TMonster::IdleStimulus(void) {
	...
	if (this->Master != 0) {
		...
		if (Master == NULL) {
			...
			ShouldDespawn = true;
		} else if (std::abs(Master->posz - this->posz) > 1 || std::abs(Master->posx - this->posx) > 30 ||
	...
	if (this->Target != 0) {
		...
		bool LoseTarget = (Target == NULL)
						  // RANGE CHECK
						  || Target->posz != this->posz || std::abs(Target->posx - this->posx) > 10 ||
	...
	if (race_data[this->Race].Talks > 0 && (rand() % 50) == 0) {
		...
		if (Text != 0 && Text[0] != 0) {
			...
			error("TMonster::IdleStimulus: Race %d talk entry %d is invalid.", this->Race, TalkNr);
		}
	}
	...
	if (this->Target == 0) {
		...
		if (this->Master == 0) {
			...
			while (true) {
				...
				if ((Target->posz != this->posz || DistanceX > 10 || DistanceY > 10) ||
					...
					(Target->IsInvisible() && !race_data[this->Race].SeeInvisible) ||
					is_protection_zone(Target->posx, Target->posy, Target->posz) ||
					is_house(Target->posx, Target->posy, Target->posz) || std::abs(Target->posx - this->posx) > 10 ||
					std::abs(Target->posy - this->posy) > 10) {
					continue;
				}
		...
		if (ShouldSleep && this->Target == 0 && this->State != UNDERATTACK && this->State != PANIC) {
			if (this->Master == 0) {
				this->State = SLEEPING;
			} else {
				this->ToDoWait(1000);
	...
	if (race_data[this->Race].Spells > 0) {
		TCreature *Target = get_creature(this->Target);
		for (int SpellNr = 1; SpellNr <= race_data[this->Race].Spells; SpellNr += 1) {
				...
				if (!Impact->isAggressive() || (this->Target != 0 && this->Target != this->Master)) {
					...
					case SHAPE_VICTIM: {
						if (Target != NULL) {
							...
							victim_shape_spell(this, Target, Range, Animation, Impact, Effect);
						}
						break;
					}
					...
					case SHAPE_DESTINATION: {
						if (Target != NULL) {
							...
							destination_shape_spell(this, Target, Range, Animation, Radius, Impact, Effect);
						}
						break;
					}
	...
	try {
		if (Target != NULL) {
			if (this->IsFleeing()) {
				...
				// NOTE(fusion): Max distance.
				int Distance = std::max<int>(std::abs(Target->posx - this->posx), std::abs(Target->posy - this->posy));
				if (!race_data[this->Race].DistanceFighting ||
					!throw_possible(this->posx, this->posy, this->posz, Target->posx, Target->posy, Target->posz, 0)) {
					...
					if (Distance > 1) {
						...
						int DestDistance =
							std::max<int>(std::abs(Target->posx - DestX), std::abs(Target->posy - DestY));
				...
				if (this->State == ATTACKING || this->State == PANIC) {
					...
					if (this->Master != this->Target) {
						...
						error("TMonster::IdleStimulus: %s attacks master %u (St=%d, T: %u).\n", this->Name,
							  this->Master, this->State, this->Target);
					}
				} else {
					this->ToDoWait(1000);
				}
...
bool TMonster::CanKickBoxes(void) {
	...
	if (!Result && this->Master != 0) {
		TCreature *Master = get_creature(this->Master);
		Result = (Master != NULL && Master->Type == MONSTER && ((TMonster *)Master)->CanKickBoxes());
	}
	return Result;
}
