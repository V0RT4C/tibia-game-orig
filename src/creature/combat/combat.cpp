...
void TCombat::CheckCombatValues(void) {
	...
	this->GetWeapon();
	this->GetAmmo();
	if (OldShield != this->Shield || OldClose != this->Close || OldMissile != this->Missile ||
		OldThrow != this->Throw || OldWand != this->Wand || OldAmmo != this->Ammo || OldFist != this->Fist) {
...
void TCombat::Attack(void) {
	...
	if (Range == 1) {
		...
		// each coordinate.
		if (std::abs(Master->posx - Target->posx) > 7 || std::abs(Master->posy - Target->posy) > 5) {
			throw TARGETOUTOFRANGE;
		}
...
void TCombat::WandAttack(TCreature *Target) {
	...
	TCreature *Master = this->Master;
	int Distance = std::max<int>(std::abs(Master->posx - Target->posx), std::abs(Master->posy - Target->posy));
	...
	if (!throw_possible(Master->posx, Master->posy, Master->posz, Target->posx, Target->posy, Target->posz, 0)) {
		throw TARGETHIDDEN;
	}
...
void TCombat::DistanceAttack(TCreature *Target) {
	...
	bool Hit = Master->Skills[SKILL_DISTANCE]->Probe(Difficulty * 15, HitChance, this->LearningPoints > 0);
	if (this->LearningPoints > 0) {
		this->LearningPoints -= 1;
	}
	...
	if (Hit) {
		...
		if (!coordinate_flag(DropX, DropY, DropZ, BANK) || coordinate_flag(DropX, DropY, DropZ, UNLAY) ||
	...
	if (SpecialEffect == 1) { // POISON ARROW
		...
		circle_shape_spell(Master, DropX, DropY, DropZ, INT_MAX, ANIMATION_NONE, 2, &Impact, EFFECT_FIRE_BURST);
	}
...
uint32 TCombat::GetMostDangerousAttacker(void) {
	...
	uint32 MaxDamage = 0;
	for (int i = 0; i < NARRAY(this->CombatList); i += 1) {
		if ((RoundNr - this->CombatList[i].TimeStamp) < 60 && this->CombatList[i].Damage > MaxDamage) {
