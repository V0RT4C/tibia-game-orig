...
bool Object::exists(void) {
	...
	return HashTableType[EntryIndex] == STATUS_LOADED && HashTableData[EntryIndex]->ObjectID == this->ObjectID;
}
	...
	while (true) {
		...
		if (strcmp(Identifier, "sectorxmin") == 0) {
			...
			*MarkArray.at(Marks) = TempMark;
			...
			Script.read_coordinate(&NewbieStartPositionX, &NewbieStartPositionY, &NewbieStartPositionZ);
		} else if (strcmp(Identifier, "veteranstart") == 0) {
			Script.read_coordinate(&VeteranStartPositionX, &VeteranStartPositionY, &VeteranStartPositionZ);
		} else {
			// TODO(fusion):
...
static void ResizeHashTable(void) {
	...
	error("FATAL ERROR in ResizeHashTable: Resizing the object hash table is"
		  ...
		  " from %d to %d, to achieve the same effect.",
		  OldSize, NewSize);
	abort();
	...
	for (int SectorZ = SectorZMin; SectorZ <= SectorZMax; SectorZ += 1)
		for (int SectorY = SectorYMin; SectorY <= SectorYMax; SectorY += 1)
			for (int SectorX = SectorXMin; SectorX <= SectorXMax; SectorX += 1) {
				...
				if (CurrentSector != NULL && CurrentSector->Status == STATUS_LOADED &&
...
void load_sector(const char *FileName, int SectorX, int SectorY, int SectorZ) {
	if (SectorX < SectorXMin || SectorXMax < SectorX || SectorY < SectorYMin || SectorYMax < SectorY ||
...
void save_map(void) {
	...
	for (int SectorZ = SectorZMin; SectorZ <= SectorZMax; SectorZ += 1)
		for (int SectorY = SectorYMin; SectorY <= SectorYMax; SectorY += 1)
			for (int SectorX = SectorXMin; SectorX <= SectorXMax; SectorX += 1) {
				snprintf(FileName, sizeof(FileName), "%s/%04d-%04d-%02d.sec", MAPPATH, SectorX, SectorY, SectorZ);
...
void refresh_sector(int SectorX, int SectorY, int SectorZ, TReadStream *Stream) {
	...
	if (SectorX < SectorXMin || SectorXMax < SectorX || SectorY < SectorYMin || SectorYMax < SectorY ||
		SectorZ < SectorZMin || SectorZMax < SectorZ) {
		error("refresh_sector: Sector %d/%d/%d is out of bounds.", SectorX, SectorY, SectorZ);
		return;
	}
...
void patch_sector(int SectorX, int SectorY, int SectorZ, bool FullSector, ReadScriptFile *Script, bool SaveHouses) {
	if (SectorX < SectorXMin || SectorXMax < SectorX || SectorY < SectorYMin || SectorYMax < SectorY ||
		SectorZ < SectorZMin || SectorZMax < SectorZ) {
		error("patch_sector: Sector %d/%d/%d is out of bounds.", SectorX, SectorY, SectorZ);
		return;
	}
		...
		while (true) {
			...
			if (Script->Token == BYTES) {
				...
				if (!House && coordinate_flag(CoordX, CoordY, CoordZ, HOOKSOUTH)) {
					House = is_house(CoordX - 1, CoordY + 1, CoordZ) || is_house(CoordX, CoordY + 1, CoordZ) ||
							is_house(CoordX + 1, CoordY + 1, CoordZ);
				}
				if (!House && coordinate_flag(CoordX, CoordY, CoordZ, HOOKEAST)) {
					House = is_house(CoordX + 1, CoordY - 1, CoordZ) || is_house(CoordX + 1, CoordY, CoordZ) ||
							is_house(CoordX + 1, CoordY + 1, CoordZ);
				}
	...
	snprintf(FileNameBak, sizeof(FileNameBak), "%s/%04d-%04d-%02d.sec~", ORIGMAPPATH, SectorX, SectorY, SectorZ);
...
void init_map(void) {
	...
	SectorMatrix = new matrix3d<Sector *>(SectorXMin, SectorXMax, SectorYMin, SectorYMax, SectorZMin, SectorZMax, NULL);
...
void place_object(Object Obj, Object Con, bool Append) {
	...
	if (ConType.is_map_container()) {
		...
		Append = Append || (ObjPriority != PRIORITY_CREATURE && ObjPriority != PRIORITY_LOW);

		while (Cur != NONE) {
			int CurPriority = get_object_priority(Cur);
			if (CurPriority == ObjPriority &&
				(ObjPriority == PRIORITY_BANK || ObjPriority == PRIORITY_BOTTOM || ObjPriority == PRIORITY_TOP)) {
				...
				error("place_object: Two %s objects (%d and %d) on field [%d,%d,%d].\n", PriorityString, ObjType.TypeID,
					  CurType.TypeID, CoordX, CoordY, CoordZ);
			}

			if (Append && CurPriority > ObjPriority)
				break;
			if (!Append && CurPriority >= ObjPriority)
				break;
...
bool is_on_map(int x, int y, int z) {
	...
	return SectorXMin <= SectorX && SectorX <= SectorXMax && SectorYMin <= SectorY && SectorY <= SectorYMax &&
...
bool is_premium_area(int x, int y, int z) {
	...
	bool Result = true;
	if (SectorX < 0x40C && (SectorX < 0x408 || SectorY > 0x3E6) && (SectorX < 0x406 || SectorY < 0x3F0) &&
		(SectorX < 0x405 || SectorY < 0x3F1) && (SectorX < 0x3FE || SectorY < 0x3F6)) {
...
int get_depot_number(const char *Town) {
	...
	for (int DepotNumber = DepotInfoArray.min; DepotNumber <= DepotInfoArray.max; DepotNumber += 1) {
