...
bool is_invited(uint16 HouseID, TPlayer *Player, int TimeStamp) {
	...
	return is_owner(HouseID, Player) || is_subowner(HouseID, Player, TimeStamp) || is_guest(HouseID, Player, TimeStamp);
}
...
void show_guest_list(uint16 HouseID, TPlayer *Player, char *Buffer) {
	...
	if (!is_owner(HouseID, Player) &&
		(!is_subowner(HouseID, Player, INT_MAX) || !check_right(Player->ID, PREMIUM_ACCOUNT))) {
		throw NOTACCESSIBLE;
	}
...
static Object CreateTempDepot(void) {
	...
	Object TempDepot = set_object(get_map_container(TempX, TempY, TempZ), get_special_object(DEPOT_LOCKER), 0);
	return TempDepot;
}
...
bool finish_auctions(void) {
	...
	int Ret = QueryManagerConnection->finishAuctions(&NumberOfAuctions, HouseIDs, CharacterIDs, CharacterNames, Bids);
	if (Ret != 0) {
...
bool transfer_houses(void) {
	...
	int Ret = QueryManagerConnection->transferHouses(&NumberOfTransfers, HouseIDs, NewOwnerIDs, NewOwnerNames, Prices);
	if (Ret != 0) {
	...
	for (int TransferNr = 0; TransferNr < NumberOfTransfers; TransferNr += 1) {
		...
		if (NewOwnerID != 0) {
			...
			House->LastTransition = TimeStamp;
			log_message("houses",
						"on reset: UPDATE HouseOwners SET Termination=%d,NewOwner=%u,Accepted=1,Price=%d WHERE "
						"World='%s' AND HouseID=%d;\n",
						TimeStamp, NewOwnerID, TransferPrice, HelpWorld, HouseID);
		} else {
			log_message("houses",
						"on reset: UPDATE HouseOwners SET Termination=%d,NewOwner=null,Accepted=0,Price=0 WHERE "
						"World=\'%s\' AND HouseID=%d;\n",
...
bool evict_ex_guild_leaders(void) {
	...
	if (NumberOfGuildHouses > 0) {
		...
		int Ret = QueryManagerConnection->evictExGuildleaders(NumberOfGuildHouses, &NumberOfEvictions, HouseIDs,
															  GuildLeaders);
		if (Ret != 0) {
...
void process_rent(void) {
	...
	if (!transfer_houses() || !evict_free_accounts() || !evict_deleted_characters() || !evict_ex_guild_leaders()) {
		return;
	}
...
bool update_house_owners(void) {
	...
	int Ret = QueryManagerConnection->getHouseOwners(&NumberOfOwners, HouseIDs, OwnerIDs, OwnerNames, PaidUntils);
	if (Ret != 0) {
...
void load_houses(void) {
	...
	int Ret = QueryManagerConnection->insertHouses(Houses, HouseIDs, Names, Rents, Descriptions, Sizes, PositionsX,
												   PositionsY, PositionsZ, Towns, Guildhouses);
	if (Ret != 0) {
