...
int pack_absolute_coordinate(int x, int y, int z) {
	...
	int Packed = (((x - 24576) & 0x00003FFF) << 18) | (((y - 24576) & 0x00003FFF) << 4) | (z & 0x0000000F);
	return Packed;
}
...
int pack_relative_coordinate(int x, int y, int z) {
	...
	int Packed = (((x + 8192) & 0x00003FFF) << 18) | (((y + 8192) & 0x00003FFF) << 4) | ((z + 8) & 0x0000000F);
	return Packed;
}
...
void change_object(Object Obj, ObjectType NewType, uint32 Value) {
	...
	for (int Attempt = 0; true; Attempt += 1) {
		...
		try {
			...
			} else if (r == NOROOM && Obj.get_object_type().get_flag(CUMULATIVE) && Obj.get_attribute(AMOUNT) > 1) {
		...
		try {
			...
			if (r != DESTROYED) {
				...
				error("move_all_objects: Exception %d; object %d, destination coordinate [%d,%d,%d].\n", r,
					  ObjType.TypeID, DestX, DestY, DestZ);
			}
		}
	}
}
...
void delete_all_objects(Object Obj, Object Exclude, bool DeleteUnmovable) {
	...
	if (Obj != Exclude) {
		...
		if (!DeleteUnmovable && ObjType.get_flag(UNMOVE) && !ObjType.get_flag(LIQUIDPOOL) &&
...
void clear_field(Object Obj, Object Exclude) {
	...
	for (int i = 0; i < 4; i += 1) {
		...
		DestZ = ObjZ;
		if (coordinate_flag(DestX, DestY, DestZ, BANK) && !coordinate_flag(DestX, DestY, DestZ, UNPASS)) {
			DestFound = true;
			break;
		}
	}
...
void send_mail(Object Obj) {
	...
	ObjectType ObjType = Obj.get_object_type();
	if (ObjType != get_special_object(PARCEL_NEW) && ObjType != get_special_object(LETTER_NEW)) {
		return;
	}
...
void send_mails(TPlayerData *PlayerData) {
	...
	for (int i = 0; i < DelayedMails; i += 1) {
		...
		if (PlayerData->Depot[DepotNr] != NULL) {
			...
			memcpy(&NewDepot[Mail->PacketSize], PlayerData->Depot[DepotNr], PlayerData->DepotSize[DepotNr]);
...
void execute_action(MoveUseEventType EventType, MoveUseAction *Action, Object User, Object Obj1, Object Obj2,
	...
	try {
		...
		case MOVEUSE_ACTION_WRITENAME: {
			...
			if (WriterObj == NONE || !WriterObj.get_object_type().is_creature_container() || TargetObj == NONE ||
				!TargetObj.get_object_type().get_flag(TEXT)) {
				throw ERROR;
			}
		...
		case MOVEUSE_ACTION_DELETEONMAP: {
			...
			if (Obj != NONE) {
				bool IsUseEvent = (EventType == MOVEUSE_EVENT_USE || EventType == MOVEUSE_EVENT_MULTIUSE);
				delete_op(Obj, (IsUseEvent ? 1 : -1));
			} else {
				error("execute_action (DELETEONMAP): No object %d at [%d,%d,%d].\n", Type.TypeID, CoordX, CoordY,
					  CoordZ);
			}
			break;
		}
...
bool handle_event(MoveUseEventType EventType, Object User, Object Obj1, Object Obj2) {
	...
	for (int RuleNr = 1; RuleNr <= DB->NumberOfRules; RuleNr += 1) {
		...
		for (int ConditionNr = Rule->FirstCondition; ConditionNr <= Rule->LastCondition; ConditionNr += 1) {
		...
		if (Execute) {
			for (int ActionNr = Rule->FirstAction; ActionNr <= Rule->LastAction; ActionNr += 1) {
...
void use_chest(uint32 CreatureID, Object Chest) {
	...
	for (int i = 0; i < 2 && !TreasureMoved; i += 1) {
		for (int Position = INVENTORY_FIRST; Position <= INVENTORY_LAST && !TreasureMoved; Position += 1) {
			try {
...
void use_liquid_container(uint32 CreatureID, Object Obj, Object Dest) {
	...
	if (!DestType.is_creature_container() || Dest.get_creature_id() != CreatureID) {
		...
		int DestX, DestY, DestZ;
		get_object_coordinates(Dest, &DestX, &DestY, &DestZ);
		if (!coordinate_flag(DestX, DestY, DestZ, BANK) || coordinate_flag(DestX, DestY, DestZ, UNLAY)) {
			throw NOROOM;
		}

		change(Obj, CONTAINERLIQUIDTYPE, LIQUID_NONE);
		create_pool(get_map_container(DestX, DestY, DestZ), get_special_object(BLOOD_POOL), LiquidType);
		return;
	}
...
void use_name_door(uint32 CreatureID, Object Door) {
	...
	if (!is_owner(HouseID, Player) && !check_right(CreatureID, OPEN_NAMEDOORS) && !may_open_door(Door, Player)) {
		throw NOTACCESSIBLE;
	}
...
void use_change_object(uint32 CreatureID, Object Obj) {
	...
	if (!ObjType.get_flag(UNPASS) && !ChangeTarget.is_map_container() && ChangeTarget.get_flag(UNPASS)) {
		clear_field(Obj, NONE);
	} else if (!ObjType.get_flag(UNLAY) && !ChangeTarget.is_map_container() && ChangeTarget.get_flag(UNLAY)) {
...
void separation_event(Object Obj, Object Start) {
	...
	while (Help != NONE) {
		...
		if (Help != Obj) {
			ObjectType HelpType = Help.get_object_type();
			if (ObjType.is_creature_container() && (HelpType.get_flag(LEVELDOOR) || HelpType.get_flag(QUESTDOOR))) {
				ObjectType DoorTarget = HelpType.get_flag(LEVELDOOR) ? (int)HelpType.get_attribute(LEVELDOORTARGET)
...
void collision_event(Object Obj, Object Dest) {
	...
	if (ObjType.get_flag(TELEPORTABSOLUTE) || ObjType.get_flag(TELEPORTRELATIVE)) {
		...
		if (ObjType.get_flag(TELEPORTABSOLUTE)) {
			unpack_absolute_coordinate((int)Obj.get_attribute(ABSTELEPORTDESTINATION), &TeleportDestX, &TeleportDestY,
									   &TeleportDestZ);
			...
			unpack_relative_coordinate((int)ObjType.get_attribute(RELTELEPORTDISPLACEMENT), &DisplacementX,
			...
			if (Help != Obj) {
				...
				if (HelpType.get_flag(TELEPORTABSOLUTE) || HelpType.get_flag(TELEPORTRELATIVE)) {
					...
					if (HelpType.get_flag(TELEPORTABSOLUTE)) {
						unpack_absolute_coordinate((int)Help.get_attribute(ABSTELEPORTDESTINATION), &TeleportDestX,
												   &TeleportDestY, &TeleportDestZ);
						...
						unpack_relative_coordinate((int)HelpType.get_attribute(RELTELEPORTDISPLACEMENT), &DisplacementX,
												   &DisplacementY, &DisplacementZ);
						TeleportDestX += DisplacementX;
						TeleportDestY += DisplacementY;
						TeleportDestZ += DisplacementZ;
...
void load_condition(ReadScriptFile *Script, MoveUseCondition *Condition) {
	...
	if (strcmp(Identifier, "isposition") == 0) {
		...
		load_parameters(Script, Condition->Parameters, 2, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_COORDINATE);
	} else if (strcmp(Identifier, "istype") == 0) {
		Condition->Condition = MOVEUSE_CONDITION_ISTYPE;
		load_parameters(Script, Condition->Parameters, 2, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_TYPE);
	} else if (strcmp(Identifier, "iscreature") == 0) {
		Condition->Condition = MOVEUSE_CONDITION_ISCREATURE;
		load_parameters(Script, Condition->Parameters, 1, MOVEUSE_PARAMETER_OBJECT);
		...
		load_parameters(Script, Condition->Parameters, 2, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_FLAG);
	} else if (strcmp(Identifier, "hastypeattribute") == 0) {
		Condition->Condition = MOVEUSE_CONDITION_HASTYPEATTRIBUTE;
		load_parameters(Script, Condition->Parameters, 4, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_TYPEATTRIBUTE,
						MOVEUSE_PARAMETER_COMPARISON, MOVEUSE_PARAMETER_NUMBER);
	} else if (strcmp(Identifier, "hasinstanceattribute") == 0) {
		Condition->Condition = MOVEUSE_CONDITION_HASINSTANCEATTRIBUTE;
		load_parameters(Script, Condition->Parameters, 4, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_INSTANCEATTRIBUTE,
						MOVEUSE_PARAMETER_COMPARISON, MOVEUSE_PARAMETER_NUMBER);
	} else if (strcmp(Identifier, "hastext") == 0) {
		Condition->Condition = MOVEUSE_CONDITION_HASTEXT;
		load_parameters(Script, Condition->Parameters, 2, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_TEXT);
		...
		load_parameters(Script, Condition->Parameters, 2, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_NUMBER);
	} else if (strcmp(Identifier, "haslevel") == 0) {
		Condition->Condition = MOVEUSE_CONDITION_HASLEVEL;
		load_parameters(Script, Condition->Parameters, 3, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_COMPARISON,
						MOVEUSE_PARAMETER_NUMBER);
	} else if (strcmp(Identifier, "hasright") == 0) {
		Condition->Condition = MOVEUSE_CONDITION_HASRIGHT;
		load_parameters(Script, Condition->Parameters, 2, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_RIGHT);
	} else if (strcmp(Identifier, "hasquestvalue") == 0) {
		Condition->Condition = MOVEUSE_CONDITION_HASQUESTVALUE;
		load_parameters(Script, Condition->Parameters, 4, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_NUMBER,
						MOVEUSE_PARAMETER_COMPARISON, MOVEUSE_PARAMETER_NUMBER);
	} else if (strcmp(Identifier, "testskill") == 0) {
		Condition->Condition = MOVEUSE_CONDITION_TESTSKILL;
		load_parameters(Script, Condition->Parameters, 4, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_SKILL,
						MOVEUSE_PARAMETER_NUMBER, MOVEUSE_PARAMETER_NUMBER);
		...
		load_parameters(Script, Condition->Parameters, 3, MOVEUSE_PARAMETER_COORDINATE, MOVEUSE_PARAMETER_COMPARISON,
						MOVEUSE_PARAMETER_NUMBER);
	} else if (strcmp(Identifier, "isobjectthere") == 0) {
		Condition->Condition = MOVEUSE_CONDITION_ISOBJECTTHERE;
		load_parameters(Script, Condition->Parameters, 2, MOVEUSE_PARAMETER_COORDINATE, MOVEUSE_PARAMETER_TYPE);
	} else if (strcmp(Identifier, "iscreaturethere") == 0) {
		Condition->Condition = MOVEUSE_CONDITION_ISCREATURETHERE;
		load_parameters(Script, Condition->Parameters, 1, MOVEUSE_PARAMETER_COORDINATE);
		...
		load_parameters(Script, Condition->Parameters, 3, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_TYPE,
						MOVEUSE_PARAMETER_NUMBER);
		...
		load_parameters(Script, Condition->Parameters, 2, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_OBJECT);
		...
		load_parameters(Script, Condition->Parameters, 1, MOVEUSE_PARAMETER_NUMBER);
	} else {
...
void load_action(ReadScriptFile *Script, MoveUseAction *Action) {
	...
	if (strcmp(Identifier, "createonmap") == 0) {
		...
		load_parameters(Script, Action->Parameters, 3, MOVEUSE_PARAMETER_COORDINATE, MOVEUSE_PARAMETER_TYPE,
						MOVEUSE_PARAMETER_NUMBER);
	} else if (strcmp(Identifier, "create") == 0) {
		Action->Action = MOVEUSE_ACTION_CREATE;
		load_parameters(Script, Action->Parameters, 3, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_TYPE,
						MOVEUSE_PARAMETER_NUMBER);
	} else if (strcmp(Identifier, "monsteronmap") == 0) {
		Action->Action = MOVEUSE_ACTION_MONSTERONMAP;
		load_parameters(Script, Action->Parameters, 2, MOVEUSE_PARAMETER_COORDINATE, MOVEUSE_PARAMETER_NUMBER);
	} else if (strcmp(Identifier, "monster") == 0) {
		Action->Action = MOVEUSE_ACTION_MONSTER;
		load_parameters(Script, Action->Parameters, 2, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_NUMBER);
		...
		load_parameters(Script, Action->Parameters, 3, MOVEUSE_PARAMETER_COORDINATE, MOVEUSE_PARAMETER_TEXT,
						MOVEUSE_PARAMETER_NUMBER);
	} else if (strcmp(Identifier, "text") == 0) {
		Action->Action = MOVEUSE_ACTION_TEXT;
		load_parameters(Script, Action->Parameters, 3, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_TEXT,
						MOVEUSE_PARAMETER_NUMBER);
	} else if (strcmp(Identifier, "changeonmap") == 0) {
		Action->Action = MOVEUSE_ACTION_CHANGEONMAP;
		load_parameters(Script, Action->Parameters, 4, MOVEUSE_PARAMETER_COORDINATE, MOVEUSE_PARAMETER_TYPE,
						MOVEUSE_PARAMETER_TYPE, MOVEUSE_PARAMETER_NUMBER);
		...
		load_parameters(Script, Action->Parameters, 5, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_VECTOR,
						MOVEUSE_PARAMETER_TYPE, MOVEUSE_PARAMETER_TYPE, MOVEUSE_PARAMETER_NUMBER);
	} else if (strcmp(Identifier, "setattribute") == 0) {
		Action->Action = MOVEUSE_ACTION_SETATTRIBUTE;
		load_parameters(Script, Action->Parameters, 3, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_INSTANCEATTRIBUTE,
						MOVEUSE_PARAMETER_NUMBER);
		...
		load_parameters(Script, Action->Parameters, 3, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_NUMBER,
						MOVEUSE_PARAMETER_NUMBER);
	} else if (strcmp(Identifier, "damage") == 0) {
		Action->Action = MOVEUSE_ACTION_DAMAGE;
		load_parameters(Script, Action->Parameters, 4, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_OBJECT,
						MOVEUSE_PARAMETER_NUMBER, MOVEUSE_PARAMETER_NUMBER);
	} else if (strcmp(Identifier, "setstart") == 0) {
		Action->Action = MOVEUSE_ACTION_SETSTART;
		load_parameters(Script, Action->Parameters, 2, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_COORDINATE);
		...
		load_parameters(Script, Action->Parameters, 2, MOVEUSE_PARAMETER_TEXT, MOVEUSE_PARAMETER_OBJECT);
	} else if (strcmp(Identifier, "logout") == 0) {
		Action->Action = MOVEUSE_ACTION_LOGOUT;
		load_parameters(Script, Action->Parameters, 1, MOVEUSE_PARAMETER_OBJECT);
	} else if (strcmp(Identifier, "moveallonmap") == 0) {
		Action->Action = MOVEUSE_ACTION_MOVEALLONMAP;
		load_parameters(Script, Action->Parameters, 2, MOVEUSE_PARAMETER_COORDINATE, MOVEUSE_PARAMETER_COORDINATE);
		...
		load_parameters(Script, Action->Parameters, 2, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_VECTOR);
		...
		load_parameters(Script, Action->Parameters, 3, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_OBJECT,
						MOVEUSE_PARAMETER_VECTOR);
	} else if (strcmp(Identifier, "retrieve") == 0) {
		Action->Action = MOVEUSE_ACTION_RETRIEVE;
		load_parameters(Script, Action->Parameters, 3, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_VECTOR,
						MOVEUSE_PARAMETER_VECTOR);
	} else if (strcmp(Identifier, "deleteallonmap") == 0) {
		Action->Action = MOVEUSE_ACTION_DELETEALLONMAP;
		load_parameters(Script, Action->Parameters, 1, MOVEUSE_PARAMETER_COORDINATE);
	} else if (strcmp(Identifier, "deletetoponmap") == 0) {
		Action->Action = MOVEUSE_ACTION_DELETETOPONMAP;
		load_parameters(Script, Action->Parameters, 2, MOVEUSE_PARAMETER_COORDINATE, MOVEUSE_PARAMETER_TYPE);
		...
		load_parameters(Script, Action->Parameters, 2, MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_OBJECT);
	} else if (strcmp(Identifier, "loaddepot") == 0) {
		Action->Action = MOVEUSE_ACTION_LOADDEPOT;
		load_parameters(Script, Action->Parameters, 4, MOVEUSE_PARAMETER_COORDINATE, MOVEUSE_PARAMETER_TYPE,
						MOVEUSE_PARAMETER_OBJECT, MOVEUSE_PARAMETER_NUMBER);
...
void load_database(void) {
	...
	while (Script.Token != ENDOFFILE) {
		...
		MoveUseRule *Rule = MoveUseDatabases[EventType].Rules.at(MoveUseDatabases[EventType].NumberOfRules);
