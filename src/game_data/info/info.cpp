...
const char *get_liquid_name(int LiquidType) {
	...
	case LIQUID_MILK:
		...
		break;
	case LIQUID_MANA:
		LiquidName = "manafluid";
		break;
	case LIQUID_LIFE:
		LiquidName = "lifefluid";
		break;
...
uint8 get_liquid_color(int LiquidType) {
	...
	case LIQUID_NONE:
		...
		break;
	case LIQUID_WATER:
		LiquidColor = LIQUID_BLUE;
		break;
	case LIQUID_WINE:
		LiquidColor = LIQUID_PURPLE;
		break;
	case LIQUID_BEER:
		LiquidColor = LIQUID_BROWN;
		break;
	case LIQUID_MUD:
		LiquidColor = LIQUID_BROWN;
		break;
	case LIQUID_BLOOD:
		LiquidColor = LIQUID_RED;
		break;
	case LIQUID_SLIME:
		LiquidColor = LIQUID_GREEN;
		break;
	case LIQUID_OIL:
		LiquidColor = LIQUID_BROWN;
		break;
	case LIQUID_URINE:
		LiquidColor = LIQUID_YELLOW;
		break;
	case LIQUID_MILK:
		LiquidColor = LIQUID_WHITE;
		break;
	case LIQUID_MANA:
		LiquidColor = LIQUID_PURPLE;
		break;
...
bool object_in_range(uint32 CreatureID, Object Obj, int Range) {
	...
	return Creature->posz == ObjZ && std::abs(Creature->posx - ObjX) <= Range &&
...
bool object_accessible(uint32 CreatureID, Object Obj, int Range) {
	...
	if (ObjType.get_flag(HANG)) {
		...
		if (HookSouth || HookEast) {
			...
			if (HookSouth) {
				if (Creature->posy < ObjY || Creature->posy > (ObjY + Range) || Creature->posx < (ObjX - Range) ||
			...
			if (HookEast) {
				if (Creature->posx < ObjX || Creature->posx > (ObjX + Range) || Creature->posy < (ObjY - Range) ||
...
int object_distance(Object Obj1, Object Obj2) {
	...
	if (ObjZ1 == ObjZ2) {
		Distance = std::max<int>(std::abs(ObjX1 - ObjX2), std::abs(ObjY1 - ObjY2));
	}
	return Distance;
}
...
Object get_top_object(int x, int y, int z, bool move) {
	...
	if (Obj != NONE) {
		while (true) {
			...
			ObjectType ObjType = Obj.get_object_type();
			if (!ObjType.get_flag(BANK) && !ObjType.get_flag(CLIP) && !ObjType.get_flag(BOTTOM) &&
				!ObjType.get_flag(TOP) && (!move || !ObjType.is_creature_container())) {
				break;
			}
...
Object get_object(uint32 CreatureID, int x, int y, int z, int RNum, ObjectType Type) {
	...
	// any object found.
	if (Obj != NONE && !Type.is_map_container() && Obj.get_object_type().get_disguise() != Type) {
		Obj = NONE;
	}
...
bool search_free_field(int *x, int *y, int *z, int Distance, uint16 HouseID, bool Jump) {
	...
	while (CurrentDistance <= Distance) {
		...
		if (Jump) {
			...
			MovePossible =
				coordinate_flag(FieldX, FieldY, FieldZ, BANK) && !coordinate_flag(FieldX, FieldY, FieldZ, UNPASS);
...
bool search_login_field(int *x, int *y, int *z, int Distance, bool Player) {
	...
	if (search_free_field(x, y, z, Distance, HouseID, false) && (!Player || !is_no_logout_field(*x, *y, *z))) {
		return true;
	}
	...
	while (true) {
		...
		for (int OffsetY = -Distance; OffsetY <= Distance; OffsetY += 1)
			for (int OffsetX = -Distance; OffsetX <= Distance; OffsetX += 1) {
				...
				int FieldY = *y + OffsetY;
				int FieldZ = *z;
				if (is_house(FieldX, FieldY, FieldZ) &&
					(HouseID == 0 || get_house_id(FieldX, FieldY, FieldZ) != HouseID)) {
					continue;
				}
						...
						for (int NeighborOffsetX = OffsetX - 1; NeighborOffsetX <= OffsetX + 1; NeighborOffsetX += 1) {
							if (-Distance <= NeighborOffsetX && NeighborOffsetX <= Distance &&
								...
								if (*NeighborPhase > ExpansionPhase) {
									*NeighborPhase = ExpansionPhase + std::abs(NeighborOffsetX - OffsetX) +
...
bool search_flight_field(uint32 FugitiveID, uint32 PursuerID, int *x, int *y, int *z) {
	...
	if (OffsetX >= 0)
		Dir[1] = DIRECTION_EAST;
	if (OffsetY <= 0)
		Dir[2] = DIRECTION_NORTH;
	if (OffsetX <= 0)
		Dir[3] = DIRECTION_WEST;
	if (OffsetY >= 0)
		Dir[4] = DIRECTION_SOUTH;
	RandomShuffle(&Dir[1], 4);

	// NOTE(fusion): Fallback to diagonal direction away from the pursuer.
	if (OffsetY <= OffsetX)
		Dir[5] = DIRECTION_NORTHEAST;
	if (OffsetY <= -OffsetX)
		Dir[6] = DIRECTION_NORTHWEST;
	if (OffsetY >= OffsetX)
		Dir[7] = DIRECTION_SOUTHWEST;
	if (OffsetY >= -OffsetX)
		Dir[8] = DIRECTION_SOUTHEAST;
...
bool search_summon_field(int *x, int *y, int *z, int Distance) {
	...
	for (int OffsetY = -Distance; OffsetY <= Distance; OffsetY += 1)
		for (int OffsetX = -Distance; OffsetX <= Distance; OffsetX += 1) {
			...
			int FieldY = *y + OffsetY;
			int FieldZ = *z;
			if (coordinate_flag(FieldX, FieldY, FieldZ, BANK) && !coordinate_flag(FieldX, FieldY, FieldZ, UNPASS) &&
				!coordinate_flag(FieldX, FieldY, FieldZ, AVOID) && !is_protection_zone(FieldX, FieldY, FieldZ) &&
				!is_house(FieldX, FieldY, FieldZ) && throw_possible(*x, *y, *z, FieldX, FieldY, FieldZ, 0)) {
				BestX = FieldX;
				BestY = FieldY;
				BestTieBreaker = TieBreaker;
			}
...
bool throw_possible(int OrigX, int OrigY, int OrigZ, int DestX, int DestY, int DestZ, int Power) {
	...
	int MaxT = std::max<int>(std::abs(DestX - OrigX), std::abs(DestY - OrigY));
...
void get_creature_light(uint32 CreatureID, int *Brightness, int *Color) {
	...
	int OutColor = 0;
	if (OutBrightness > 0) {
		OutColor = (OutRed / OutBrightness) * 36 + (OutGreen / OutBrightness) * 6 + (OutBlue / OutBrightness);
	}
...
bool check_banishment_right(uint32 CharacterID, int Reason, int Action) {
	...
	if (check_right(CharacterID, (RIGHT)(Reason + 18))) {
		...
		case 3: {
			Result = Name && check_right(CharacterID, NAMELOCK) && check_right(CharacterID, BANISHMENT);
			break;
		}

		case 4: {
			Result = !Name && check_right(CharacterID, BANISHMENT) && check_right(CharacterID, FINAL_WARNING);
			break;
		}
