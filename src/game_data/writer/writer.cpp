...
void log_message(const char *ProtocolName, const char *Text, ...) {
	...
	bool WriteDate = (strcmp(ProtocolName, "bugreport") != 0) && (strcmp(ProtocolName, "client-error") != 0) &&
	...
	if (WriteDate) {
		...
		snprintf(Line, sizeof(Line), "%02d.%02d.%04d %02d:%02d:%02d (%u): %s", LocalTime.tm_mday, LocalTime.tm_mon + 1,
				 LocalTime.tm_year + 1900, LocalTime.tm_hour, LocalTime.tm_min, LocalTime.tm_sec, RoundNr, Output);
	} else {
		strcpy(Line, Output);
	}
...
void process_playerlist_order(PlayerlistOrderData *Data) {
	...
	if (Data->NumberOfPlayers <= 0) {
		int Ret = QueryManagerConnection->createPlayerlist(Data->NumberOfPlayers, NULL, NULL, NULL, &NewRecord);
		if (Ret != 0) {
		...
		int Ret =
			QueryManagerConnection->createPlayerlist(Data->NumberOfPlayers, Names, Levels, Professions, &NewRecord);
		if (Ret != 0) {
...
void process_kill_statistics_order(KillStatisticsOrderData *Data) {
	...
	int Ret =
		QueryManagerConnection->logKilledCreatures(Data->NumberOfRaces, RaceNames, KilledPlayers, KilledCreatures);
	if (Ret != 0) {
...
static const char *GetStatementOutputChannel(int Mode, int Channel) {
	...
	case TALK_YELL:
		Result = "Yell";
		break;
	case TALK_PRIVATE_MESSAGE:
		Result = "Private Message";
		break;
	case TALK_GAMEMASTER_REQUEST:
		Result = "Gamemaster Request";
		break;
	case TALK_GAMEMASTER_ANSWER:
		Result = "Gamemaster Answer";
		break;
	case TALK_PLAYER_ANSWER:
		Result = "Player Answer";
		break;
	case TALK_GAMEMASTER_BROADCAST:
		Result = "Broadcast";
		break;
	case TALK_GAMEMASTER_MESSAGE:
		Result = "Private Message";
		break;
	...
	case TALK_HIGHLIGHT_CHANNELCALL: {
		...
		case CHANNEL_GAMEMASTER:
			Result = "Gamemaster Channel";
			break;
		case CHANNEL_TUTOR:
			Result = "Tutor Channel";
			break;
		case CHANNEL_GAMECHAT:
			Result = "Game Chat";
			break;
		case CHANNEL_TRADE:
			Result = "Trade Channel";
			break;
		case CHANNEL_RLCHAT:
			Result = "Reallife Chat";
			break;
		case CHANNEL_HELP:
			Result = "Help Channel";
			break;
		default:
			Result = "Private Chat Channel";
			break;
		}
		break;
	}
...
void process_punishment_order(PunishmentOrderData *Data) {
	...
	if (Ok && Data->Action == 0) {
		int Ret = QueryManagerConnection->setNotation(Data->GamemasterID, Data->CriminalName, Data->CriminalIPAddress,
													  Reason, Data->Comment, &BanishmentID);
		switch (Ret) {
		case 0: {
		...
		case 1: {
			direct_reply(Data->GamemasterID,
	...
	if (Ok && (Data->Action == 1 || Data->Action == 3 || Data->Action == 5)) {
		int Ret = QueryManagerConnection->setNamelock(Data->GamemasterID, Data->CriminalName, Data->CriminalIPAddress,
													  Reason, Data->Comment);
		switch (Ret) {
		case 0: {
		...
		case 1: {
			direct_reply(Data->GamemasterID,
						 "A player with this name does not exist. Perhaps he/she has already been renamed?");
			Ok = false;
			break;
		}
	...
	if (Ok && (Data->Action == 2 || Data->Action == 3 || Data->Action == 4 || Data->Action == 5)) {
		...
		int Ret = QueryManagerConnection->banishAccount(Data->GamemasterID, Data->CriminalName, Data->CriminalIPAddress,
														Reason, Data->Comment, &FinalWarning, &Days, &BanishmentID);
		switch (Ret) {
		case 0: {
			if (Days == -1) {
				direct_reply(Data->GamemasterID, "Account of player %s banished infinitely.", Data->CriminalName);
			} else if (FinalWarning) {
				direct_reply(Data->GamemasterID, "Account of player %s banished for %d days with final warning.",
							 Data->CriminalName, Days);
			} else {
				direct_reply(Data->GamemasterID, "Account of player %s banished for %d days.", Data->CriminalName,
							 Days);
			}
	...
	if (Ok && Data->StatementID != 0) {
		if (Data->NumberOfStatements > 0 && Data->ReportedStatements != NULL) {
			...
			int Ret = QueryManagerConnection->reportStatement(
				Data->GamemasterID, Data->CriminalName, Reason, Data->Comment, BanishmentID, Data->StatementID,
				Data->NumberOfStatements, StatementIDs, TimeStamps, CharacterIDs, Channels, Texts);
			switch (Ret) {
			case 0: {
				if (Data->Action == 6) {
	...
	if (Ok && Data->ip_banishment) {
		int Ret = QueryManagerConnection->banishIPAddress(Data->GamemasterID, Data->CriminalName,
														  Data->CriminalIPAddress, Reason, Data->Comment);
		switch (Ret) {
		case 0: {
...
void process_character_death_order(CharacterDeathOrderData *Data) {
	...
	int Ret = QueryManagerConnection->logCharacterDeath(Data->CharacterID, Data->Level, Data->Offender, Data->Remark,
														Data->Unjustified, Data->Time);
	if (Ret != 0) {
